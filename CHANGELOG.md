...
- An SSH provider that runs vendor CLIs on remote hosts without installing accel there.
- Fleet mode: `--listen` serves `/api/snapshot`, `/api/summary`, `/api/events`, `/api/history`, and Prometheus `/metrics`; `--remote` attaches a TUI to another accel.
