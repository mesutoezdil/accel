#!/bin/sh
# Rasterizes the screenshots README.md embeds, at 2x, with headless Chrome,
# so they look the same in every viewer. The rest stay SVG.
set -eu
chrome="${CHROME:-/Applications/Google Chrome.app/Contents/MacOS/Google Chrome}"
[ -x "$chrome" ] || { echo "png.sh: no Chrome at $chrome, skipping" >&2; exit 0; }
for name in overview devices history links health; do
	f="assets/$name.svg"
